...
void PrintProgramPurpose() {
  ...
  std::cout << "Este programa sirve para calcular el número más alto de un vector, el más bajo y el promedio" << std::endl << std::endl;
}
...
/**
  * @brief Esta función saca el máximo, el mínimo y el promedio de un vector de números
  *
  * @param vector Vector al que realizar las operaciones
  * @param maximo Número máximo que hay en el vector
  * @param minimo Número más pequeño que hay en el vector
  * @param promedio Media de los números del vector
  */

void MaximoMinimoPromedioVector(std::vector<double>& vector, double& maximo, double& minimo, double& promedio) {

  maximo = vector[0];
  minimo = vector[0];
  double suma_valores_vector{};

  for (const auto& value: vector) {

    suma_valores_vector += value;
  ...
  promedio = suma_valores_vector / static_cast<int>(vector.size());
}
